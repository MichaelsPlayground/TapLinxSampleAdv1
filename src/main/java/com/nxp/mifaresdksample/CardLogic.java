...
import static com.nxp.mifaresdksample.Constants.DEFAULT_ICode_PAGE;
import static com.nxp.mifaresdksample.Constants.DEFAULT_SECTOR_CLASSIC;
import static com.nxp.mifaresdksample.Constants.KEY_AES128_DEFAULT;
import static com.nxp.mifaresdksample.Constants.TAG;
import static com.nxp.mifaresdksample.Constants.UNABLE_TO_READ;
...
import static com.nxp.mifaresdksample.MainActivity.mString;
...
//             connect to card, authenticate and read data
  ...
  */
    ...
    // appended
    String desfireEV3CardLogic(MainActivity activity, IDESFireEV3 desFireEV3) {
        int timeOut = 2000;
        stringBuilder.delete(0, stringBuilder.length());
        stringBuilder.append(activity.getString(R.string.Card_Detected)).append(
                desFireEV3.getType().getTagName()).append(activity.getString(R.string.LINE_BREAK));

        stringBuilder.append(activity.getString(R.string.UID)).append(
                Utilities.dumpBytes(desFireEV3.getUID())).append(
                activity.getString(R.string.LINE_BREAK));

        try {
            desFireEV3.getReader().setTimeout(timeOut);
            stringBuilder.append(activity.getString(R.string.SIZE)).append(
                    desFireEV3.getTotalMemory()).append(activity.getString(R.string.LINE_BREAK));
            ...
            byte[] getVersion = desFireEV3.getVersion();
            if (getVersion[0] == (byte) 0x04) {
                stringBuilder.append(activity.getString(R.string.Vendor_ID)).append(
                        activity.getString(R.string.NXP)).append(
                        activity.getString(R.string.LINE_BREAK));
            ...
            int[] app_Ids = desFireEV3.getApplicationIDs();
            for (int app_id : app_Ids) {
                byte[] ids = Utilities.intToBytes(app_id, 3);
                String str = Utilities.byteToHexString(ids);
                stringBuilder.append(activity.getString(R.string.Application_IDs)).append(
            ...
            desFireEV3.authenticate(0, IDESFireEV1.AuthType.Native, KeyType.THREEDES,
                    objKEY_2KTDES);
            stringBuilder.append(activity.getString(R.string.Auth_success)).append(
                    activity.getString(R.string.LINE_BREAK));
